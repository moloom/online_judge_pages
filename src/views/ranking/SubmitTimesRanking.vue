<template>
  <div>
    <div style="margin-left: 70px;height: 30px;margin-top: 30px;font-size: 20px;">提交次数榜</div>
    <table class="table">
      <tr class="th">
        <td style="width: 10%;">#</td>
        <td style="width: 30%;">题目名称</td>
        <td style="width: 10%;">提交数量</td>
...
export default {
  ...
  methods: {
    searchSubmitTimesRank(start) {
      axios({
        url: "/rank/searchSubmitTimesRank",
        method: "post",
        data: qs.stringify({
          ...
          error => {
            console.log("searchSubmitTimesRank 请求失败", error);
